...
def printBTree(node, nodeInfo=None, inverted=False, isTop=True, path = ''):

    # node value string and sub nodes
    if isinstance(node,bytes) :
        stringValue, leftNode, rightNode = path + ' : ' + node.__str__(), None, None
    else :
        stringValue = path[-1] if len(path) > 0 else '*'
        leftNode, rightNode = nodeInfo(node)
        
    stringValueWidth  = len(stringValue)

    # recurse to sub nodes to obtain line blocks on left and right
    leftTextBlock     = [] if not leftNode else printBTree(leftNode,nodeInfo,inverted,False, path+'0')

    rightTextBlock    = [] if not rightNode else printBTree(rightNode,nodeInfo,inverted,False, path+'1')
